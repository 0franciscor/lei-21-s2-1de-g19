...
* Question: Will there be a need to link each sample and the collection method(s) used? If so, is it associated when creating a test (US4) or when registering the samples collected (US5)? And is(are) the collection method(s) associated with the sample or with each of the parameters? [Client forum](https://moodle.isep.ipp.pt/mod/forum/discuss.php?d=8260)
    * Answer:
* Question: What are the necessary parameters according to the type of test? [Client forum](https://moodle.isep.ipp.pt/mod/forum/discuss.php?d=8294)
    * Answer:
...
| Step 2: request data (citizenID)  		 | n/a							 |             |                              |
| Step 3: types requested data  		 | confirms the imput data?							 | ClientStore            | IE: knows all the client objects                              |
| Step 4: shows the client's information and request a confirmation  		 | n/a 							 |             |                              |
| Step 5: confirms the data 		 | ... confirms the client's information?							 | ClientStore             | IE: knows all the client information                             |
...
| Step 8: show all parameters according to the test type and ask to select one or more  		 | n/a							 |             |                              |              
| Step 9: selects the parameter(s) to be analysed   		 | ...saving the selected parameter(s)?						 | Test             | IE: object created in step 1 has one or more parameters.                              |              
| Step 10: show all data and request a confirmation  		 | ... validating the data locally (e.g.: mandatory vs.non-mandatory data)?							 | Test             | IE: owns its data                              |
|  		 | … validating the data globally (e.g.: duplicated)? 							 | TestStore             | IE: knows all the test objects                              |
| Step 11: confirms the data 		 | ... saving the registered test? 							 | TestStore             | IE: records all the test objects                              |
| Step 12: register test and informs operation success  		 | ... informing operation success?							 | RegisterTestUI            | IE: responsible for user interaction                              |
