...
|:-------------  |:--------------------- |:------------|:---------------------------- |
| Step 1: register a test to be performed to a registred client  		 | ... interacting with the actor?							 |             |                              |
|  		 | ... coordinating the US?							 |             |                              |
|   		 | ... register a test?							 |             |                              |
| Step 2: request data (citizenID)  		 | n/a							 |             |                              |
| Step 3: types requested data  		 | confirms the imput data?							 |             |                              |
| Step 4: shows the test types and ask to select one  		 | n/a							 |             |                              |
| Step 5: selects the test type  		 | ...saving the inputted data?							 |             |                              |
| Step 6: show all parameters according to the test type and ask to select one or more  		 | n/a							 |             |                              |              
| Step 7: selects the parameter(s) to be analysed   		 | ...saving the inputted data?							 |             |                              |              
| Step 8: show all data and request a confirmation  		 | ... validating the data locally (e.g.: mandatory vs.non-mandatory data)?							 |             |                              |
|  		 | … validating the data globally (e.g.: duplicated)? 							 |             |                              |
| Step 9: confirms the data 		 | ... saving the registered test? 							 |             |                              |
| Step 10: register test and informs operation success  		 | ... informing operation success?							 |             |                              |
