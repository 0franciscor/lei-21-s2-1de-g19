...
|:-------------  |:--------------------- |:------------|:---------------------------- |
| Step 1: register a type of test | ...creating a new type of test? | TestTypeStore | Creator: The TestTypeStore is responsible for creating a test type| feito
|                                 | ...coordinating the US?  | createTestTypeController | Controller| feito
| Step 2: request data(code, description, collectingMethod) | ...requesting the data? | createTestTypeUI | IE: responsible for user interaction | feito
| Step 3: types requested data | ...saving the input data? | TestTypeStore | IE: The TestTypeStore is responsible for saving the new test type, since it was responsible for creating it and will validate it | feito
| Step 4: validates the data, shows it and requests a confirmation | ... validating the data locally (e.g.: mandatory vs.non-mandatory data)? | TestTypeStore | IE: is able to validate a new Test Type, according to the provided Acceptance Criteria | feito
|                                                                  | ... validating the data globally (e.g.: duplicated)? | Company  | IE: knows all TestType objects | feito
| Step 5: confirms the data | ... saving the typed data? | TestTypeStore | Creator: saves all the TestType objects | feito
| Step 6: shows a list of Parameter Categories | ...showing the available options? | createTestTypeUI and ParameterCategoryStore| IE and Creator: UI is responsible for showing it and the Parameter Category Store is able to retrieve the available categories |
| Step 7: selects a category from the list | ...making the list available? | createTestTypeUI | IE: allows user to insert the intended option |
| Step 8: shows the selected category | ... saving the selected data? | createTestTypeUI | IE: responsible for user interaction |
| Step 9: confirms it |  ... saving the created type of test? | TestTypeStore | Creator: records all the TestType objects | 
| Step 10: adds test to list and informs operation success | ...informing addition success? | createTestTypeUI | IE: responsible for user interaction | feito
