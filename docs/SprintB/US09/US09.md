...
* Question: Which attributes does a type of test have besides its name and Collecting Method?
    * Answer: A code, a description,a Collecting Method and a set of Parameter Category. [Client Forum](https://moodle.isep.ipp.pt/mod/forum/discuss.php?d=7512#p10171)
...
|:-------------  |:--------------------- |:------------|:---------------------------- |
| Step 1: register a type of test | ...creating a new type of test? | Administrator | Creator: in the DM, Administrator creates a test type|
|                                 | ...coordinating the US?  | createTestTypeController | Controller|
| Step 2: request data(code, description, collectingMethod) | ...requesting the ~~~~data? | n/a | n/a |
| Step 3: types requested data | ...saving the input data? | TestType | IE: the object created in step 1 has its own data |
| Step 4: validates the data, shows it and requests a confirmation | ... validating the data locally (e.g.: mandatory vs.non-mandatory data)? | Administrator | IE: knows all the required data |
|                                                                  | ... validating the data globally (e.g.: duplicated)? | Company | IE: knows all TestType objects |
| Step 5: confirms the data | ... saving the created type of test? | Company | IE: records all the TestType objects |
| Step 6: adds test to list and informs operation success| ...informing addition success? | UI | IE: responsible for user interaction |             
