...
| Step 4: shows the existing parameter categories and asks to select one		 |	...	knowing the task categories to show?			 |   createParameterUI and ParameterCategoryStore          |  IE and Creator: UI is responsible for showing it and the Parameter Category Store is able to retrieve the available categories                            |
